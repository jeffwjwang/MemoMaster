  ...
  **视觉重构准则：**
  ...
  switch (type) {
    case '灵感记录':
      ...
      3. **应用场景**：使用 "bento" 块展示具体的应用场景。
      4. **落地指南**：使用 "step" 块给出这个灵感的落地的可行性分析及分步骤的**具体行动指南**（不设上限，确保每一步都详尽、可执行）。`;
      break;
    case '会议纪要':
      systemInstruction += `\n**【会议纪要专项指令：深度捕获与全量还原】**
    ...
    case '读图':
      ...
      2. **深度知识解读**：使用 "text" 或 "bento" 块，进行结构化、内容丰富的知识解读。不设篇幅限制，以“讲明白、讲透彻”为准。
      3. **术语与概念详解**：使用 "bento" 块，针对图中出现的**重要概念、专业术语或当前流行的词汇**展开详细的背景说明和原理解释，确保用户能够完全理解其深层含义。
