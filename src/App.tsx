  ...
  **视觉重构准则：**
  ...
  switch (type) {
    case '灵感记录':
      ...
      2. **新颖性与价值分析**：使用 "bento" 块，深度分析这个灵感是否具有**新颖性**（是否是市面上罕见的、独特的视角）以及**实际运用价值**（能否解决具体问题、是否有商业或生活化落地的潜力）。
    ...
    case '好文收藏':
      ...
      - 如果是专业图表（流程图、数据图表、论文插画）：解析其逻辑结构或核心趋势。使用 step 块还原流程逻辑（不设上限，确保详尽还原每一个逻辑节点），使用 bento 块列出关键数据点或发现。
      - 综合分析：使用 bento 块分析用户收藏此内容的深层意图（如：审美积累、逻辑参考、数据佐证等）。`;
      break;
    case '读图':
      systemInstruction += `\n**【读图专项指令：高信息密度图表深度解析模式】**
      你的目标是解析用户上传的知识图谱、架构图、流程图或任何高信息密度的图片。
      ...
      6. **应用技巧与注意点**：使用 "step" 块，给出在上述场景中应用此图的专业技巧、讲解逻辑或注意事项（不设上限，确保详尽、专业）。
