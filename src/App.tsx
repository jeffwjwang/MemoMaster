...
// --- Types ---
type MemoType = '灵感记录' | '会议纪要' | '读图' | '任务清单' | '英语拾贝' | '好文收藏';
const MEMO_TYPES: MemoType[] = ['灵感记录', '会议纪要', '读图', '任务清单', '英语拾贝', '好文收藏'];
...
async function analyzeMemo(
  ...
  **视觉重构准则：**
  ...
  2. **块数量无上限**：针对 "bento"、"list"、"step" 等块，严禁为了排版而合并或压缩内容。必须以“描述清楚、不遗漏任何细节”为原则，根据内容量生成足够多的块（不设数量上限）。
  3. **严格字数限制（单块内容）**：
     - "text" (叙述文本): 每块文字内容严禁超过 200 字。
     - "bento" (便当格): 每项细节描述严禁超过 20 字。
     - "list" (列表): 每个列表项严禁超过 20 字。
     - "step" (步骤): 每个步骤的描述严禁超过 30 字。
  4. **视觉层次**：通过交替使用不同的块类型(type) and 颜色(color)来创造“杂志排版”感。
  5. **标题索引**：每个块必须配上一个精准、有吸引力的“小标题(title)”。

  可用块类型：
  1. "highlight": 用于核心洞察、金句或最重要的结论。
  2. "quote": 用于摘录感性、优美或具有代表性的原文片段。
  3. "step": 用于逻辑推导、时间线 or 有先后顺序的思考。
  ...
  switch (type) {
    case '灵感记录':
      systemInstruction += `\n**【灵感记录专项指令：创意孵化与价值评估模式】**
      1. **核心灵感**：使用 "highlight" 块展示核心灵感及用户想到这个灵感的意图。
      2. **新颖性与价值分析**：使用 "bento" 块，深度分析这个灵感是否具有**新颖性**（是否是市面上罕见的、独特的视角）以及**实际运用价值**（能否解决具体问题、是否有商业 or 生活化落地的潜力）。
      3. **应用场景**：使用 "bento" 块展示具体的应用场景（不设数量上限，穷举所有潜在可能）。
    ...
    case '英语拾贝':
      ...
      1.必须提供不超过3个对应这个中文核心短语对应的英语书面表达短语及口语表达短语
      2.必须提供这些英文短语的应用注意点，并给出对应的例句以及其中文翻译`;
      break;
    case '好文收藏':
      systemInstruction += `\n重点：
      - 如果是文字内容：使用 highlight 块提取金句，使用 quote 块摘录精彩片段。
      - 如果是图片内容（漫画、照片、插画）：分析其视觉隐喻、情感内核或艺术风格，使用 highlight 块总结其“一眼万年”的精髓。
      - 如果是专业图表（流程图、数据图表、论文插画）：解析其逻辑结构或核心趋势。使用 step 块还原流程逻辑（不设上限，确保详尽还原每一个逻辑节点），使用 bento 块列出关键数据点 or 发现。
      - 综合分析：使用 bento 块分析用户收藏此内容的深层意图（如：审美积累、逻辑参考、数据佐证等）。`;
      break;
    case '读图':
      systemInstruction += `\n**【读图专项指令：高信息密度图表深度解析模式】**
      你的目标是解析用户上传的知识图谱、架构图、流程图 or 任何高信息密度的图片。
      
      1. **一句话概括**：使用 "highlight" 块，用极其精炼的一句话总结图中的核心知识。
      2. **深度知识解读**：使用 "text" 或 "bento" 块，进行结构化、内容丰富的知识解读。以“讲明白、讲透彻、不遗漏”为准，若内容较多请拆分为多个块。
      3. **术语与概念详解**：使用 "bento" 块，针对图中出现的**重要概念、专业术语或当前流行的词汇**展开详细的背景说明和原理解释（确保用户能够完全理解其深层含义，若解释较长请拆分为多个 bento 项）。
      4. **核心要点提炼**：使用 "list" 块，列出图中最重要的 3-5 个核心知识点。
      5. **应用场景建议**：使用 "bento" 块，分析这张图可以应用在什么场合（如：演讲引用、写书创作、方案汇报、教学演示等）。
      6. **应用技巧与注意点**：使用 "step" 块，给出在上述场景中应用此图的专业技巧、讲解逻辑 or 注意事项（不设上限，确保详尽、专业）。
      7. **高情商分享语**：使用 "quote" 块，提供一段简短、深刻的说明文字，方便用户在分享此图时展现出极高的专业素养和洞察力。
      
      **视觉要求**：确保每个块的标题（title）精准且具有吸引力。颜色分配要体现出专业感（多用蓝色、紫色、灰色）。`;
      break;
  }
...
function MemoCreator({ onClose, onSave }: { onClose: () => void; onSave: (memo: any) => void }) {
  ...
  const handleAudioFileUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    const file = e.target.files?.[0];
    if (file) {
      // Check size: if > 15MB, inform the user about compression
      if (file.size > 15 * 1024 * 1024) {
        alert("音频文件较大，AI 整理时将自动为您进行智能压缩以确保分析成功。");
      }
  ...
  const startRecording = async () => {
    try {
      ...
      mediaRecorder.onstop = async () => {
        ...
        // Check size: if > 15MB, inform the user
        if (audioBlob.size > 15 * 1024 * 1024) {
          alert("录音文件较大，AI 整理时将自动为您进行智能压缩以确保分析成功。");
        }
